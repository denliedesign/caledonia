<ul class="navbar-nav text-center">
    <li class="nav-item mx-2">
        <a class="nav-link" href="/">Home</a>
    </li>
    <li class="nav-item mx-2">
        <a class="nav-link" href="/about-us">About</a>
    </li>
    <li class="nav-item mx-2">
        <a class="nav-link" href="/staff">Staff</a>
    </li>
    <li class="nav-item mx-2">
        <a class="nav-link" href="/fall">Fall</a>
    </li>
    <li class="nav-item mx-2">
        <a class="nav-link" style="line-height: 0.75;" href="/nutcracker">Nutcracker<br><span class="text-muted" style="font-size: 0.65em;">TICKETS AVAILABLE</span></a>
    </li>
{{--    <li class="nav-item mx-2">--}}
{{--        <a class="nav-link" href="/class-schedule">Summer</a>--}}
{{--    </li>--}}
    <li class="nav-item mx-2">
        <a class="nav-link" href="/calendar">Calendar</a>
    </li>
    <li class="nav-item mx-2">
        <a class="nav-link" href="/dress-code">Attire</a>
    </li>
    <li class="nav-item mx-2">
        <a class="nav-link" href="/fall">Register</a>
{{--        <a class="nav-link" href="https://app.jackrabbitclass.com/regv2.asp?id=383292" target="_blank">Register</a>--}}
    </li>
    <li class="nav-item mx-2">
        <a class="nav-link" href="https://app.jackrabbitclass.com/jr3.0/ParentPortal/Login?orgId=383292&regcid=11480783" target="_blank">Login</a>
    </li>
{{--    <li class="nav-item mx-2">--}}
{{--        <a class="nav-link" href="https://caledonia-dance-music-center.myshopify.com" target="_blank">Shop</a>--}}
{{--    </li>--}}
</ul>
